...
This Apify actor scrapes job listings from Careerone using Crawlee's CheerioCrawler and gotScraping.

## Features

- Scrapes Careerone search results and job detail pages (no browser required).
...
- `location` (string) — Location filter (e.g., "Sydney NSW"). Default: "Sydney NSW".
- `category` (string) — Job category to filter (if available on Careerone).
...
- If Careerone changes their markup, selectors in `src/main.js` may need small updates.